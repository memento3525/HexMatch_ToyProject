...
namespace Mentum.HexMatch
{
    public class GameManager : MonoBehaviour
    {
        ...
        private bool CheckAndSlide(Block block)
        {
            ...
            Point upCheck = curPoint + Point.Up;
        ...
        private void SpawnFromSpawnPoint(Point spawnPoint)
        {
        ...
        private BlockFlipCtrl GetFlipped(Block block)
        {
            for (int i = 0; i < flippedBlocks.Count; i++)
            {
        ...
        List<Point> IsConnected(Point curPoint, bool main)
        {
            ...
            for (int i = 0; i < 3; i++) // 방향의 절반 값
            {
                ...
                Point[] check =
                {
                    curPoint + directions[i],
                    curPoint + directions[i + 3] // 반대방향을 구하기위해 +3
                };
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    Point[] check =
                    {
                        curPoint + square_directions[first],
                        curPoint + square_directions[second],
                        curPoint + square_directions[first] + square_directions[second]
                    };
        ...
        /// </summary>
        private Node.Type GetTypeAtPoint(Point p)
        {
            if (p.x < 0 || p.x >= width || p.y < 0 || p.y >= height) return Node.Type.Hole;
            return board[p.x, p.y].type;
        }

        private void SetTypeAtPoint(Point p, Node.Type t)
        {
            board[p.x, p.y].type = t;
        }

        private Node GetNodeAtPoint(Point p) => board[p.x, p.y];

        // remove와 겹치지않는 색상 한 개 랜덤으로 리턴.
        private Node.Type NewValue(ref List<Node.Type> remove)
        {
            var availableList = new List<Node.Type>();
            for (int i = 0; i < Node.colorLength; i++)
                availableList.Add((Node.Type)(i + 1));
